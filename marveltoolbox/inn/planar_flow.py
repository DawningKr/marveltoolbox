...
parser.add_argument(
    "--plot", action="store_true", help="Plot a flow and target density."
)
...
parser.add_argument(
    "--target_potential",
    choices=["u_z0", "u_z5", "u_z1", "u_z2", "u_z3", "u_z4"],
    help="Which potential function to approximate.",
)
# flow params
parser.add_argument(
    "--base_sigma",
    type=float,
    default=4,
    help="Std of the base isotropic 0-mean Gaussian distribution.",
)
parser.add_argument(
    "--learn_base",
    default=False,
    action="store_true",
    help="Whether to learn a mu-sigma affine transform of the base distribution.",
)
...
parser.add_argument(
    ...
    type=float,
    default=1,
    help="Initialization std for the trainable flow parameters.",
)
...
parser.add_argument(
    ...
    type=int,
    default=0,
    help="Starting step (if resuming training will be overwrite from filename).",
)
parser.add_argument("--n_steps", type=int, default=1000000, help="Optimization steps.")
parser.add_argument("--lr", type=float, default=1e-5, help="Learning rate.")
parser.add_argument("--weight_decay", type=float, default=1e-3, help="Weight decay.")
parser.add_argument(
    "--beta", type=float, default=1, help="Multiplier for the target potential loss."
)
...
u_z1 = lambda z: 0.5 * ((torch.norm(z, p=2, dim=1) - 2) / 0.4) ** 2 - torch.log(
    torch.exp(-0.5 * ((z[:, 0] - 2) / 0.6) ** 2)
    + torch.exp(-0.5 * ((z[:, 0] + 2) / 0.6) ** 2)
    + 1e-10
)
u_z2 = lambda z: 0.5 * ((z[:, 1] - w1(z)) / 0.4) ** 2
u_z3 = lambda z: -torch.log(
    torch.exp(-0.5 * ((z[:, 1] - w1(z)) / 0.35) ** 2)
    + torch.exp(-0.5 * ((z[:, 1] - w1(z) + w2(z)) / 0.35) ** 2)
    + 1e-10
)
u_z4 = lambda z: -torch.log(
...
def optimize_flow(base_dist, flow, target_energy_potential, optimizer, args):
    ...
    for i in range(args.start_step, args.n_steps):
        ...
        p_log_prob = -temp(i) * target_energy_potential(
            zk
        )  # p = exp(-potential) ==> p_log_prob = - potential
        ...
        if i % 10000 == 0:
            ...
            print(
                "{}: step {:5d} / {}; loss {:.3f}; base_log_prob {:.3f}, sum log dets {:.3f}, p_log_prob {:.3f}, max base = {:.3f}; max qk = {:.3f} \
                zk_mean {}, zk_sigma {}; base_mu {}, base_log_sigma {}".format(
                    args.target_potential,
                    i,
                    args.n_steps,
                    loss.item(),
                    base_log_prob.mean(0).item(),
                    sum_log_abs_det_jacobians.mean(0).item(),
                    p_log_prob.mean(0).item(),
                    base_log_prob.exp().max().item(),
                    log_qk.exp().max().item(),
                    zk.mean(0).cpu().data.numpy(),
                    zk.var(0).sqrt().cpu().data.numpy(),
                    (
                        base_dist.loc.cpu().data.numpy()
                        if not args.learn_base
                        else flow[0].mu.cpu().data.numpy()
                    ),
                    (
                        base_dist.covariance_matrix.cpu().diag().data.numpy()
                        if not args.learn_base
                        else flow[0].logsigma.cpu().data.numpy()
                    ),
                )
            )
            ...
            torch.save(
                ...
                os.path.join(
                    args.output_dir,
                    "model_state_flow_length_{}.pt".format(args.flow_length),
                ),
            )
            ...
            with torch.no_grad():
                plot_flow(
                    ...
                    flow,
                    os.path.join(
                        args.output_dir, "approximating_flow_step{}.png".format(i)
                    ),
                    args,
                )
...
def plot_flow_density(base_dist, flow, ax, range_lim=4, n=200, output_dir=None):
    ...
    if output_dir:
        plt.tight_layout()
        plt.savefig(
            os.path.join(output_dir, "flow_k{}_density.png".format(len(flow) - 1))
        )
        plt.close()
...
if __name__ == "__main__":
    ...
    # setup flow
    flow = nn.Sequential(
        AffineTransform(args.learn_base),
        *[PlanarTransform() for _ in range(args.flow_length)]
    ).to(args.device)
    ...
    base_dist = D.MultivariateNormal(
        torch.zeros(2).to(args.device), args.base_sigma * torch.eye(2).to(args.device)
    )
    ...
    if args.restore_file:
        ...
        try:
            ...
            flow = nn.Sequential(
                *[PlanarTransform(args.init_sigma) for _ in range(args.flow_length)]
            )
    ...
    if args.evaluate:
        plot_flow(
            base_dist,
            flow,
            os.path.join(args.output_dir, "approximating_flow.png"),
            args,
        )
