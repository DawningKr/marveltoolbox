...
__all__ = ["MADE", "MADEMOG", "MAF", "MADEMOG", "RealNVP"]
...
parser.add_argument(
    "--n_hidden", type=int, default=1, help="Number of hidden layers in each MADE."
)
parser.add_argument(
    "--activation_fn",
    type=str,
...
class MaskedLinear(nn.Linear):
    ...
    def extra_repr(self):
        ...
        ) + (self.cond_label_size != None) * ", cond_features={}".format(
...
class MADEMOG(nn.Module):
    ...
    def log_prob(self, x, y=None):
        u, log_abs_det_jacobian = self.forward(
            x, y
        )  # u = (N,C,L); log_abs_det_jacobian = (N,C,L)
        # marginalize cluster probs
        log_probs = torch.logsumexp(
            self.logr + self.base_dist.log_prob(u) + log_abs_det_jacobian, dim=1
        )  # sum over C; out (N, L)
...
class MAFMOG(nn.Module):
    ...
    def log_prob(self, x, y=None):
        ...
        # marginalize cluster probs
        log_probs = torch.logsumexp(
            self.mademog.logr + self.base_dist.log_prob(u) + log_abs_det_jacobian, dim=1
        )  # out (N, L)
...
def evaluate(model, dataloader, epoch, args):
    ...
    logprob_mean, logprob_std = logprobs.mean(0), 2 * logprobs.var(
        0
    ).sqrt() / math.sqrt(len(dataloader.dataset))
    output = (
        "Evaluate "
        + (epoch != None) * "(epoch {}) -- ".format(epoch)
...
def generate(model, dataset_lam, args, step=None, n_row=10):
    ...
    if args.cond_label_size:
        ...
        for i in range(args.cond_label_size):
            ...
            log_probs = (
                model.log_prob(sample, labels_i).sort(0)[1].flip(0)
    ...
    # unconditional model
        ...
        log_probs = (
            model.log_prob(samples).sort(0)[1].flip(0)
        )  # sort by log_prob; take argsort idxs; flip high to low
    ...
    save_image(
        samples, os.path.join(args.output_dir, filename), nrow=n_row, normalize=True
    )
...
def plot_sample_and_density(
    model,
    target_dist,
    args,
    ranges_density=[[-5, 20], [-10, 10]],
    ranges_sample=[[-4, 4], [-4, 4]],
    step=None,
):
    model.eval()
    ...
    plt.savefig(
        os.path.join(
            args.output_dir,
            "sample" + (step != None) * "_epoch_{}".format(step) + ".png",
        )
    )
    plt.close()
...
if __name__ == "__main__":
    ...
    if args.conditional:
        assert args.dataset in [
            "MNIST",
            "CIFAR10",
        ], "Conditional inputs only available for labeled datasets MNIST and CIFAR10."
    train_dataloader, test_dataloader = fetch_dataloaders(
        args.dataset, args.batch_size, args.device, args.flip_toy_var_order
    )
    ...
    args.cond_label_size = (
        train_dataloader.dataset.label_size if args.conditional else None
    )
    ...
    if args.model == "made":
        ...
        assert (
            args.n_components > 1
        ), "Specify more than 1 component for mixture of gaussians models."
        model = MADEMOG(
            args.n_components,
            args.input_size,
            args.hidden_size,
    ...
    if args.generate:
        if args.dataset == "TOY":
            ...
            plot_sample_and_density(
                model,
                base_dist,
                args,
                ranges_density=[[-15, 4], [-3, 3]],
