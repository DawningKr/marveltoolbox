...
parser.add_argument(
    "--generate", action="store_true", help="Generate samples from a model."
)
parser.add_argument(
    "--visualize", action="store_true", help="Visualize manipulated attribures."
)
parser.add_argument("--restore_file", type=str, help="Path to model to restore.")
parser.add_argument("--seed", type=int, help="Random seed to use.")
# paths and reporting
parser.add_argument(
    "--data_dir", default="/mnt/disks/data/", help="Location of datasets."
)
parser.add_argument(
    "--output_dir", default="./results/{}".format(os.path.splitext(__file__)[0])
)
parser.add_argument(
    "--results_file",
    default="results.txt",
...
parser.add_argument(
    "--save_interval", type=int, default=50, help="How often to save during training."
)
parser.add_argument(
    "--eval_interval",
    type=int,
    default=1,
    help="Number of epochs to eval model and save model checkpoint.",
)
# data
parser.add_argument("--dataset", type=str, help="Which dataset to use.")
# model parameters
parser.add_argument(
    "--depth", type=int, default=32, help="Depth of the network (cf Glow figure 2)."
)
parser.add_argument(
    "--n_levels",
    type=int,
    default=3,
    help="Number of levels of of the network (cf Glow figure 2).",
)
parser.add_argument(
    "--width", type=int, default=512, help="Dimension of the hidden layers."
)
parser.add_argument(
    "--z_std",
    type=float,
    help="Pass specific standard devition during generation/sampling.",
)
# training params
...
parser.add_argument(
    ...
    type=int,
    default=256,
    help="Batch size for the data dependent initialization.",
)
parser.add_argument(
...
parser.add_argument(
    ...
    type=int,
    default=2,
    help="Number of warmup epochs for linear learning rate annealing.",
)
parser.add_argument(
    "--start_epoch",
    default=0,
...
parser.add_argument(
    ...
    type=int,
    default=None,
    help="Train only on this number of datapoints.",
)
parser.add_argument(
    "--grad_norm_clip", default=50, type=float, help="Clip gradients during training."
)
parser.add_argument(
    "--checkpoint_grads",
    action="store_true",
    default=False,
    help="Whether to use gradient checkpointing in forward pass.",
)
parser.add_argument(
    "--n_bits", default=5, type=int, help="Number of bits for input images."
)
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Whether to use DistributedDataParallels on multiple machines and GPUs.",
)
parser.add_argument(
    "--world_size",
    type=int,
...
parser.add_argument(
    "--local_rank",
    type=int,
    help="When provided, run model on this cuda device. When None, used by torch.distributed.launch utility to manage multi-GPU training.",
)
# visualize
parser.add_argument(
    "--vis_img",
    type=str,
    help="Path to image file to manipulate attributes and visualize.",
)
parser.add_argument(
    "--vis_attrs", nargs="+", type=int, help="Which attribute to manipulate."
)
parser.add_argument(
    "--vis_alphas",
    nargs="+",
    type=float,
    help="Step size on the manipulation direction.",
)
...
def fetch_dataloader(args, train=True, data_dependent_init=False):
    ...
    transforms = {
        "mnist": T.Compose(
            [
                T.Pad(2),  # image to 32x32 same as CIFAR
                T.RandomAffine(
                    degrees=0, translate=(0.1, 0.1)
                ),  # random shifts to fill the padded pixels
                T.ToTensor(),
        ...
        "celeba": T.Compose(
            [
                T.CenterCrop(148),  # RealNVP preprocessing
                T.Resize(64),
    ...
    if (
        args.distributed and train is True and not data_dependent_init
    ):  # distributed training; but exclude initialization
        sampler = torch.utils.data.distributed.DistributedSampler(dataset)
    else:
        sampler = None

    batch_size = (
        args.batch_size_init if data_dependent_init else args.batch_size
    )  # if data dependent init use init batch size
    kwargs = (
        {"num_workers": 1, "pin_memory": True} if args.device.type == "cuda" else {}
    )
    return DataLoader(
        dataset,
...
class Actnorm(nn.Module):
    ...
    def forward(self, x):
        if not self.initialized:
            ...
            self.bias.squeeze().data.copy_(
                x.transpose(0, 1).flatten(1).mean(1)
            ).view_as(self.scale)
            self.scale.squeeze().data.copy_(
                x.transpose(0, 1).flatten(1).std(1, False) + 1e-6
            ).view_as(self.bias)
            self.initialized += 1
    ...
    def inverse(self, z):
        return (
            z * self.scale + self.bias,
            self.scale.abs().log().sum() * z.shape[2] * z.shape[3],
        )
...
class Invertible1x1Conv(nn.Module):
    ...
    def __init__(self, n_channels=3, lu_factorize=False):
        ...
        w = torch.qr(w)[
            0
        ]  # note: nn.init.orthogonal_ returns orth matrices with dets +/- 1 which complicates the inverse call below
        ...
        if lu_factorize:
            ...
            self.p, self.l, self.u = (
                nn.Parameter(p.squeeze()),
                nn.Parameter(l.squeeze()),
                nn.Parameter(u.squeeze()),
            )
            s = self.u.diag()
            ...
            self.register_buffer(
                "l_mask", torch.tril(torch.ones_like(self.l), -1)
    ...
    def inverse(self, z):
        ...
        if self.lu_factorize:
            ...
            u = torch.inverse(
                self.u * self.l_mask.t() + torch.diag(self.sign_s * self.log_s.exp())
            )
...
class AffineCoupling(nn.Module):
    ...
    def __init__(self, n_channels, width):
        ...
        self.conv1 = nn.Conv2d(
            n_channels // 2, width, kernel_size=3, padding=1, bias=False
    ...
    def forward(self, x):
        ...
        s = torch.sigmoid(
            s + 2.0
        )  # at initalization, s is 0 and sigmoid(2) is near identity
...
class Squeeze(nn.Module):
    ...
    def inverse(self, x):
        ...
        x = x.reshape(
            B, C // 4, 2 * H, 2 * W
        )  # aggregate channel dim factors into spatial dims
        return x
...
class FlowSequential(nn.Sequential):
    ...
    def forward(self, x):
        ...
        for module in self:
            x, logdet = (
                module(x) if not self.checkpoint_grads else checkpoint(module, x)
            )
...
class FlowLevel(nn.Module):
    ...
    def __init__(
        self, n_channels, width, depth, checkpoint_grads=False, lu_factorize=False
    ):
        super().__init__()
        # network layers
        self.squeeze = Squeeze()
        self.flowsteps = FlowSequential(
            *[FlowStep(4 * n_channels, width, lu_factorize) for _ in range(depth)],
...
class Glow(nn.Module):
    ...
    def __init__(
        ...
        self.flowlevels = nn.ModuleList(
            [
                FlowLevel(
                    in_channels * 2**i, width, depth, checkpoint_grads, lu_factorize
                )
                for i in range(n_levels)
            ]
        )
        self.squeeze = Squeeze()
        self.flowstep = FlowSequential(
            *[FlowStep(out_channels, width, lu_factorize) for _ in range(depth)],
            checkpoint_grads=checkpoint_grads
        )
    ...
    def inverse(self, zs=None, batch_size=None, z_std=1.0):
        if (
            zs is None
        ):  # if no random numbers are passed, generate new from the base distribution
            assert (
                batch_size is not None
            ), "Must either specify batch_size or pass a batch of z random numbers."
            zs = [
                z_std * self.base_dist.sample((batch_size, *self.output_dims)).squeeze()
            ]
        ...
        for i, m in enumerate(reversed(self.flowlevels)):
            z = z_std * (
                self.base_dist.sample(x.shape).squeeze() if len(zs) == 1 else zs[-i - 2]
            )  # if no z's are passed, generate new random numbers from the base dist
...
def data_dependent_init(model, args):
    ...
    model(
        ...
        .requires_grad_(True if args.checkpoint_grads else False)
        .to(args.device)
    )
    del dataloader
    return True
...
def train_epoch(model, dataloader, optimizer, writer, epoch, args):
    ...
    for i, (x, y) in enumerate(dataloader):
        ...
        if epoch <= args.n_epochs_warmup:
            optimizer.param_groups[0]["lr"] = args.lr * min(
                1,
                args.step / (len(dataloader) * args.world_size * args.n_epochs_warmup),
            )

        x = x.requires_grad_(True if args.checkpoint_grads else False).to(
            args.device
        )  # requires_grad needed for checkpointing
        ...
        if i % args.log_interval == 0:
            ...
            with torch.no_grad():
                zs, _ = model(x)
                kls = [
                    D.kl.kl_divergence(D.Normal(z.mean(), z.std()), model.base_dist)
                    for z in zs
                ]
            ...
            if args.on_main_process:
                ...
                print(
                    "Epoch: [{}/{}][{}/{}]\tStep: {}\tTime: elapsed {:.0f}m{:02.0f}s / total {:.0f}m{:02.0f}s\tLoss {:.4f}\t".format(
                        epoch,
                        args.start_epoch + args.n_epochs,
                        i + 1,
                        len(dataloader),
                        args.step,
                        et // 60,
        ...
        if i % args.save_interval == 0:
            # generate samples
            samples = generate(model, n_samples=4, z_stds=[0.0, 0.25, 0.7, 1.0])
            ...
            if args.on_main_process:
                save_image(
                    images,
                    os.path.join(
                        args.output_dir, "generated_sample_{}.png".format(args.step)
                    ),
                )
...
def evaluate(model, dataloader, args):
    ...
    logprob_mean, logprob_std = logprobs.mean(0), 2 * logprobs.std(0) / math.sqrt(
...
def generate(model, n_samples, z_stds):
    ...
    for z_std in z_stds:
        ...
        samples.append(
            sample[log_probs.argsort().flip(0)]
        )  # sort by log_prob; flip high (left) to low (right)
    return torch.cat(samples, 0)


def train_and_evaluate(
    model, train_dataloader, test_dataloader, optimizer, writer, args
):
    ...
    for epoch in range(args.start_epoch, args.start_epoch + args.n_epochs):
        ...
        if False:  # epoch % args.eval_interval == 0:
            ...
            print(
                "Evaluate at epoch {}: bits_x = {:.3f} +/- {:.3f}".format(
                    epoch, eval_logprob_mean, eval_logprob_std
                )
            )
...
def compute_dz(zs, attrs, idx):
    ...
    return torch.stack(z_pos).mean(0) - torch.stack(z_neg).mean(
        0
    )  # out tensor of shape (flattened zs dim,)
...
def get_manipulators(zs, attrs):
    ...
    dzs = 1.6 * torch.stack(
        [compute_dz(zs, attrs, i) for i in range(attrs.shape[1])], dim=0
    )  # compute dz for each attribute official code multiplies by 1.6 scalar here
    print("Completed.")
...
def manipulate(model, z, dz, z_std, alpha):
    ...
    z = z + dz * torch.tensor(alpha).float().view(-1, 1).to(
        dz.device
    )  # out (n_alphas, flattened zs dim)
    # 3. reshape back to z shapes from each level of the model
    zs = [
        z_.view((len(alpha), *dim)) for z_, dim in zip(z.split(z_numels, dim=1), z_dims)
    ]
    # 4. decode
...
def load_manipulators(model, args):
    ...
    if os.path.exists(os.path.join(args.output_dir, "z_manipulate.pt")):
        z_manipulate = torch.load(
            os.path.join(args.output_dir, "z_manipulate.pt"), map_location=args.device
        )
    else:
...
def visualize(model, args, attrs=None, alphas=None, img_path=None, n_examples=1):
    ...
    dataset = fetch_dataloader(
        args, train=False
    ).dataset  # pull the dataset to access transforms and attrs
    ...
    for attr_idx in attrs:
        dec_x.append(
            manipulate(
                model, z, z_manipulate[attr_idx].unsqueeze(0), args.z_std, alphas
            )
        )
...
if __name__ == "__main__":
    ...
    args.output_dir = (
        os.path.dirname(args.restore_file)
        if args.restore_file
        else os.path.join(
            args.output_dir, time.strftime("%Y-%m-%d_%H-%M-%S", time.gmtime())
        )
    )
    ...
    if args.distributed:
        ...
        )  # torch.distributed.launch sets this to nproc_per_node * nnodes
    else:
        if torch.cuda.is_available():
            args.local_rank = 0
        args.device = torch.device(
            "cuda:{}".format(args.local_rank) if args.local_rank is not None else "cpu"
        )
    ...
    args.on_main_process = (
        args.distributed and args.local_rank == 0
    ) or not args.distributed
    ...
    # load model
    model = Glow(
        args.width, args.depth, args.n_levels, args.input_dims, args.checkpoint_grads
    ).to(args.device)
    if args.distributed:
    ...
    if args.restore_file:
        ...
        optimizer.load_state_dict(
            torch.load(
                os.path.dirname(args.restore_file) + "/optim_checkpoint.pt",
    ...
    if args.generate:
        ...
        save_image(
            ...
            os.path.join(
                args.output_dir,
                "generated_samples_at_z_std_{}.png".format(
                    "range" if args.z_std is None else args.z_std
                ),
            ),
        )
    ...
    if args.visualize:
        if not args.z_std:
            args.z_std = 0.6
        if not args.vis_alphas:
            args.vis_alphas = [-2, -1, 0, 1, 2]
        dec_x = visualize(
            model, args, args.vis_attrs, args.vis_alphas, args.vis_img
        )  # output (n_attr, n_alpha, 3, H, W)
        filename = (
            "manipulated_sample"
            if not args.vis_img
            else "manipulated_img_{}".format(
                os.path.basename(args.vis_img).split(".")[0]
            )
        )
        if args.vis_attrs:
            filename += "_attr_" + ",".join(map(str, args.vis_attrs))
        save_image(
            dec_x.view(-1, *args.input_dims),
