...
class Gnet32(nn.Module):
    def __init__(self, nc, nz):
        ...
        self.main = nn.Sequential(
            # Z latent vector 100
            nn.ConvTranspose2d(
                in_channels=nz, out_channels=1024, kernel_size=4, stride=1, padding=0
            ),
            ...
            # State (256x16x16)
            nn.ConvTranspose2d(
                in_channels=256, out_channels=nc, kernel_size=4, stride=2, padding=1
            ),
        )
        # self.output = nn.Tanh()
...
class Enet32(nn.Module):
    def __init__(self, nc, nz):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(
                in_channels=nc, out_channels=256, kernel_size=4, stride=2, padding=1
            ),
