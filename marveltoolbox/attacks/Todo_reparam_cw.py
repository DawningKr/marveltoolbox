...
class ReparamCWAttack(CWAttack):
    def __init__(
        self,
        params={"p": 0.999, "confidence": 50, "threshold": 1e-4},
    ):
    ...
    def detective_loss(self, rec_x, adv_x):
        ...
        flag = torch.where(
            D >= self.threshold, torch.zeros_like(D), torch.ones_like(D)
        ).detach()
        return loss, flag
    ...
    def z_loss(self, classifier, z, labels):
        ...
        flag = torch.where(
            wrong_logit < self.p,
            torch.zeros_like(wrong_logit),
            torch.ones_like(wrong_logit),
        ).detach()
    ...
    def attack_batch(self, defense, inputs, labels):
        ...
        while abs(prev_mean_norm - mean_norm) >= self.TOLERANCE or acc <= 0.99:
            ...
            for t in range(self.T):
                ...
                loss4, success_flag3 = self.z_loss(
                    defense.encoder.classifier, new_z, labels
                )
            ...
            beta = torch.where(
                success_flag == 1, new_beta_when_success, new_beta_when_failure
            )
