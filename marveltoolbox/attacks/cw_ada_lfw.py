...
class AdaCWAttack_lfw(CWAttack):
    ...
    def attack_batch(self, defense, inputs, labels):
        ...
        lower_beta, upper_beta = (
            torch.zeros(len(x)).to(x.device),
            torch.zeros(len(x)).to(x.device) + 1e9,
        )
        ...
        binary_idx = 0
        while (abs(prev_mean_norm - mean_norm) >= self.TOLERANCE or acc <= 0.99) and (
            binary_idx < self.MAX_BINARY
        ):
            ...
            for t in range(self.T):
                ...
                scores, loss_list, success_flag_list = defense(
                    (adv_x, x_t), is_return_more=True
                )
            ...
            idx = (
                ((success_flag == 1).view(-1) * (D < best_adv_norm).view(-1))
                .nonzero()
                .view(-1)
            )
            ...
            if self.is_cwloss:
                ...
                beta = torch.where(
                    success_flag_cw == 1, new_beta_when_success, new_beta_when_failure
                )
            ...
            if conf_flag:
                for i in range(loss_num):
                    ...
                    upper_beta_list[i] = torch.where(
                        success_flag_list[i] == 1, beta_list[i], upper_beta_list[i]
                    )
                    lower_beta_list[i] = torch.where(
                        success_flag_list[i] == 0, beta_list[i], lower_beta_list[i]
                    )
                    beta_list[i] = torch.where(
                        success_flag_list[i] == 1,
                        new_beta_when_success,
                        new_beta_when_failure,
                    )
