...
class Trainer(mt.BaseTrainer, Confs):
    def __init__(self):
        ...
        self.models["G"] = mt.nn.mine.Gnet(self.nz, output_size=self.input_size).to(
            self.device
        )
        self.models["D"] = mt.nn.mine.Dnet(self.input_size, output_size=1).to(
            self.device
        )
        self.models["M"] = mt.nn.mine.Mine(
            noise_size=self.nz_m, sample_size=self.input_size, output_size=1
        ).to(self.device)

        self.optims["G"] = torch.optim.Adam(
            self.models["G"].parameters(), lr=self.lr, betas=(self.beta1, self.beta2)
        )

        self.optims["D"] = torch.optim.Adam(
            self.models["D"].parameters(), lr=self.lr, betas=(self.beta1, self.beta2)
        )

        self.optims["M"] = torch.optim.Adam(
            self.models["M"].parameters(), lr=self.lr, betas=(self.beta1, self.beta2)
        )

        self.train_loader, self.val_loader, self.test_loader, _ = mt.datasets.load_data(
            self.dataset, 1.0, 1.0, self.batch_size, self.img_size, None, False
        )
    ...
    def train(self, epoch):
        for i in range(self.iters_per_epoch):
            ...
            probs_fake = self.models["D"](x_fake)
            mi_loss = self.MI_object(
                z[:, 0 : self.nz_m], z_b, x_fake, is_train_mi=False
            )
...
def update_target(ma_net, net, update_rate=1e-1):
    ...
    for ma_net_param, net_param in zip(ma_net.parameters(), net.parameters()):
        ma_net_param.data.copy_(
            (1.0 - update_rate) * ma_net_param.data + update_rate * net_param.data
        )
